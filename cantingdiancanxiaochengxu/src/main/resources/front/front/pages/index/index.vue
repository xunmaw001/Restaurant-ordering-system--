<template>
	<view class="uni-padding-wrap">
		...
		<view class="pl15 col3 header-title" style="padding: 0 12px;box-sizing: border-box;"
			:style='{"backgroundColor":"#07588D","borderColor":"#4477C5","color":"rgba(255, 255, 255, 1.0)","borderRadius":"30rpx 30rpx 0px 0px","borderWidth":"4rpx","fontSize":"32rpx","lineHeight":"72rpx","borderStyle":"solid  solid none solid"}'>
		...
		<view class="pl15 col3 header-title" style="padding: 0 12px;box-sizing: border-box;"
			...
			<view class="right-content" @tap="onPageTap('goods')"
				:style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1.0)'}">
				查看更多
			</view>
		</view>
		<view v-if='2 == 2' class="uni-product-list" :class="2 == 2 ? 'active' : ''"
			:style='{"borderRadius":"0","backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap('goods',product.id)" :style='{"borderRadius":"80rpx","backgroundColor":"#4477C5"}'
				class="uni-product" v-for="(product,index) in goodsList2" :key="index">
				<view :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#fff","textAlign":"center"}'
					class="uni-product-title">{{product.goodsName}}</view>
				<view class="image-view" :style='{"borderRadius":"2rpx","height":"180rpx"}'>
					<image :style='{"height":"180rpx","borderRadius":"90rpx"}' mode="aspectFill"
		...
		<view v-if='2 == 2' class="uni-product-list" :class="2 == 2 ? 'active' : ''"
			:style='{"borderRadius":"0","backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap('goods',product.id)" :style='{"borderRadius":"80rpx","backgroundColor":"#4477C5"}'
				class="uni-product" v-for="(product,index) in goodsList" :key="index">
				<view :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#fff","textAlign":"center"}'
					class="uni-product-title">{{product.goodsName}}</view>
				<view class="image-view" :style='{"borderRadius":"2rpx","height":"180rpx"}'>
					<image :style='{"height":"180rpx","borderRadius":"90rpx"}' mode="aspectFill"
...
<script>
	...
	export default {
		...
		async onShow() {
			...
			//热销推荐
			let goods2 = await this.$api.list('goods', {
			 page: 1,
				limit: 6,
				orderBy: 'shangxia_xiaoliang',
				shangxiaTypes: 1,
				goodsDelete: 1,
			});
			this.goodsList2 = goods2.data.list
